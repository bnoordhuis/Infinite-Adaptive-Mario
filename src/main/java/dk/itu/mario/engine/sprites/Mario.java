...
public class Mario extends Sprite
{
	...
	public static String levelString = "50";
	...
	public void move()
	{
		if(deathTime == 0 && winTime == 0){
			...
			{
				if(LevelScene.recorder != null && LevelScene.recorder.recording)
					LevelScene.recorder.startDuckRecord();
			}
			else
			{
				if(LevelScene.recorder != null && LevelScene.recorder.recording && !ducking)
					LevelScene.recorder.endDuckRecord();
			}
		}
		...
		if(deathTime == 0 && winTime == 0){
			if(keys[KEY_SPEED]){
				if(LevelScene.recorder!= null && LevelScene.recorder.recording){
					LevelScene.recorder.startRunningRecord();
				}
				...
				if(LevelScene.recorder!= null && LevelScene.recorder.recording){
					LevelScene.recorder.endRunningRecord();
				}
		...
		if (onGround)
		{
			...
			if(LevelScene.recorder != null)
				LevelScene.recorder.recordJumpLand();
		}
		...
		if (keys[KEY_JUMP] || (jumpTime < 0 && !onGround && !sliding))
		{
			...
			else if (onGround && mayJump)
			{
				...
				if(LevelScene.recorder != null){
					LevelScene.recorder.isInAir = true;
					LevelScene.recorder.recordJump();
				}
			}
		...
		if(world.mario.xa > 0){
			...
			if(direction != 1){
				...
				if(LevelScene.recorder != null){
					...
					LevelScene.recorder.startRightMoveRecord();
				}
			}
			...
			if(direction != -1){
				...
				if(LevelScene.recorder != null){
					//        			world.recorder.switchRecord();
					LevelScene.recorder.startLeftMoveRecord();
				}
			}
			...
			//was moving right
			if(direction == 1 && LevelScene.recorder!= null){
				LevelScene.recorder.endRightMoveRecord();
			}
			//was moving left
			else if(direction == -1 && LevelScene.recorder!= null){
				LevelScene.recorder.endLeftMoveRecord();
			}
		...
		if (carried != null)
		{
			...
			if (!keys[KEY_SPEED])
			{
				...
				if(LevelScene.recorder != null){
					LevelScene.recorder.shellUnleashedRecord();
				}
			}
		}
	}
	...
	private boolean isBlocking(float _x, float _y, float xa, float ya)
	{
		...
		if (((LevelInterface.TILE_BEHAVIORS[block & 0xff]) & LevelInterface.BIT_PICKUPABLE) > 0)
		{
			if(LevelScene.recorder != null)
				LevelScene.recorder.recordCoin();
	...
	public void stomp(Enemy enemy)
	{
		...
		if(LevelScene.recorder!=null)
			LevelScene.recorder.killStompRecord(enemy);
	}
	...
	public void stomp(Shell shell)
	{
		...
		else
		{
			...
			if(shell.xa == 0 && LevelScene.recorder!= null){
				LevelScene.recorder.shellUnleashedRecord();
			}
		}
	}
	...
	public void getHurt(Sprite sprite)
	{
		...
		if (large)
		{
			...
			if (fire)
			{
				...
				if(LevelScene.recorder != null){
					LevelScene.recorder.endFireRecord();
					LevelScene.recorder.startLargeRecord();
				}
			}
			else
			{
				...
				if(LevelScene.recorder != null){
					LevelScene.recorder.endLargeRecord();
					LevelScene.recorder.startLittleRecord();
				}
			}
			invulnerableTime = 32;
		}
	...
	public void dieSprite(Sprite sprite){
		...
		if(LevelScene.recorder != null){
			LevelScene.recorder.dieRecord(sprite);
		}
	}
	...
	public void dieTime(){
		...
		if(LevelScene.recorder!=null){
			LevelScene.recorder.dieTimeRecord();
		}
	}
	...
	public void dieJump(){
		...
		if(LevelScene.recorder!=null){
			LevelScene.recorder.dieJumpRecord();
		}
	}
	...
	public void die()
	{
		...
		if(LevelScene.recorder != null){
			...
			if(!large && !fire)
				LevelScene.recorder.endLittleRecord();

			if(ducking)
				LevelScene.recorder.endDuckRecord();

			LevelScene.recorder.endTime();
			LevelScene.recorder.recordJumpLand();
		}
	...
	public void getFlower()
	{
		...
		if (!fire)
		{
			...
			if(LevelScene.recorder != null){
				...
				LevelScene.recorder.startFireRecord();
			}
		}
		else
		{
	...
	public void stomp(BulletBill bill)
	{
		...
		if(LevelScene.recorder!=null)
			LevelScene.recorder.killStompRecord(bill);
	}

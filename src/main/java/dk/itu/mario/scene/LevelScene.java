...
public class LevelScene extends Scene implements SpriteContext
{
	...
	public void render(Graphics g, float alpha)
	{
		...
		if (mario.deathTime > 0)
		{
			...
			if (t > 1800)
			{
				CustomizedLevel.decreaseChallenge((int)(mario.x/16));
				Mario.lives--;
				deathActions();
			}
	...
	public void bump(int x, int y, boolean canBreakBricks)
	{
		...
		if ((LevelInterface.TILE_BEHAVIORS[block & 0xff] & LevelInterface.BIT_BUMPABLE) > 0)
		{
			bumpInto(x, y - 1);
			level.setBlock(x, y, (byte) 4);

			if (((LevelInterface.TILE_BEHAVIORS[block & 0xff]) & LevelInterface.BIT_SPECIAL) > 0)
			{
		...
		if ((LevelInterface.TILE_BEHAVIORS[block & 0xff] & LevelInterface.BIT_BREAKABLE) > 0)
		{
			bumpInto(x, y - 1);
			if (canBreakBricks)
			{

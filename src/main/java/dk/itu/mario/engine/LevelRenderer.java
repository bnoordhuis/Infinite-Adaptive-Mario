	...
	private void updateArea(int x0, int y0, int w, int h)
	{
		...
		for (int x = xTileStart; x <= xTileEnd; x++)
		{
			for (int y = yTileStart; y <= yTileEnd; y++)
			{
				...
				if (((LevelInterface.TILE_BEHAVIORS[b]) & LevelInterface.BIT_ANIMATED) == 0)
				{
	...
	public void render(Graphics g, int tick, float alpha)
	{
		...
		for (int x = xCam / 16; x <= (xCam + width) / 16; x++)
			for (int y = yCam / 16; y <= (yCam + height) / 16; y++)
			{
				...
				if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_ANIMATED) > 0)
				{
				...
				if (renderBehaviors)
				{
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_BLOCK_UPPER) > 0)
					{
						g.setColor(Color.RED);
						g.fillRect((x << 4) - xCam, (y << 4) - yCam, 16, 2);
					}
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_BLOCK_ALL) > 0)
					{
						g.setColor(Color.RED);
						g.fillRect((x << 4) - xCam, (y << 4) - yCam, 16, 2);
					...
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_BLOCK_LOWER) > 0)
					{
						g.setColor(Color.RED);
						g.fillRect((x << 4) - xCam, (y << 4) - yCam + 14, 16, 2);
					}
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_SPECIAL) > 0)
					{
						g.setColor(Color.PINK);
						g.fillRect((x << 4) - xCam + 2 + 4, (y << 4) - yCam + 2 + 4, 4, 4);
					}
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_BUMPABLE) > 0)
					{
						g.setColor(Color.BLUE);
						g.fillRect((x << 4) - xCam + 2, (y << 4) - yCam + 2, 4, 4);
					}
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_BREAKABLE) > 0)
					{
						g.setColor(Color.GREEN);
						g.fillRect((x << 4) - xCam + 2 + 4, (y << 4) - yCam + 2, 4, 4);
					}
					if (((LevelInterface.TILE_BEHAVIORS[b & 0xff]) & LevelInterface.BIT_PICKUPABLE) > 0)
					{
